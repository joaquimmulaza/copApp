...
import type { ChatMessage, GeminiChatRequest, GeminiChatResponse } from '@/types/gemini'
  ...
  const sendMessage = useCallback(
    async (content: string, chipId?: GeminiChipId) => {
      ...
      try {
        ...
        const { data } = await api.post<GeminiChatResponse>('/gemini/chat', body)
        ...
        dispatch({
          type:    'SET_ERROR',
          payload: 'Não foi possível obter resposta do assistente. Tenta novamente.',
        })
      } finally {
